...
<?php include('con_db.php');
	$lid=$_GET['loc'];
	$sql=mysql_query("select * from location where loc_id='$lid'") or die(mysql_error());
	...
	<div class="container w3ls-service" id="service">
		<div class="row-agile">
			...
			<div class="col-md-4 col-sm-4 col-xs-6 part1" style="background-color: #B7E4EB; border: 2px solid #0FA4BB; padding: 25px; border-radius: 8px; ">
				<h3 class="center">Update Location</h3>
				<form action="" method="post">
				  <div class="form-group">
				  	<label>Location</label>
					<input type="text" name="location" class="form-control" pattern="[A-Za-z]+" title="letters only" value="<?php echo $row['loc_name']; ?>" required>
				  </div>
				...
				<?php include('con_db.php');
					...
					{
						$loc=$_POST['location'];
						$qry=mysql_query("UPDATE `location` SET `loc_name`='$loc' WHERE loc_id='$lid'") or die(mysql_error());
						if($qry){
							echo '<script>alert("Updated successfully"); window.location="viewlocation.php"; </script>';
						}
						else{
