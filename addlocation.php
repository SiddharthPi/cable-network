		...
		<div class="row-agile">
			<div class="container">
			  <ul class="list-inline">
			    <li><a href="viewlocation.php" class="btn btn-warning">View Location</a></li>
			  </ul>
			</div>
			...
			<div class="col-md-4 col-sm-4 col-xs-6 part1" style="background-color: #B7E4EB; border: 2px solid #0FA4BB; padding: 25px; border-radius: 8px; ">
				...
				<form action="" method="post">
				  <div class="form-group">
					...
					<input type="text" name="location" class="form-control" pattern="[A-Za-z]+" title="letters only" placeholder="Enter Location" required>
				  </div>
				...
				<?php include('con_db.php');
					...
					{
						$loc=$_POST['location'];
						$qr=mysql_query("SELECT * FROM location where loc_name='$loc'");
						$rc=mysql_num_rows($qr);
						if($rc>0)
						{
							echo '<script>alert("Already location has been added with same name"); </script>';
						}
						else
						{
						$qry=mysql_query("INSERT INTO `location`(`loc_name`) VALUES ('$loc')") or die(mysql_error());
						if($qry)
						{
							echo '<script>alert("Inserted successfully"); window.location="viewlocation.php"; </script>';
						}
						else
						{
