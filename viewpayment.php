	...
	<div class="container w3ls-service" id="service">
		<div class="row-agile">
			...
			<div class="col-md-8 col-sm-8 col-xs-8 part1" style="background-color: #B7E4EB; border: 2px solid #0FA4BB; padding: 25px; border-radius: 8px; ">
				<h3 class="center">View Payments</h3> <br>
					  ...
					  <div class="form-group col-md-4">
						...
						<select name="cust" class="form-control">
							...
							<?php include('con_db.php');
								$qr=mysql_query("select * from user") or die(mysql_error());
								while($rw=mysql_fetch_array($qr)) {
							 ?>
							 <option value="<?php echo $rw['first_name']; ?>"><?php echo $rw['first_name']; ?></option>
							 <?php } ?>
						</select>
					  </div>
				...
				<table class="table table-bordered table-striped table-hover" style="background-color: #fff;"">
					...
					<?php include('con_db.php');
						$a=1;
						$qr=mysql_query("select * from receipt,user,receipt_pay where receipt.rid=receipt_pay.rid and user.first_name=receipt.user_name and receipt.user_name='$cust'") or die(mysql_error());
						while($fetch=mysql_fetch_array($qr))
						{
							?>
							<tr>
								<td><?php echo $a++; ?></td>
								<td><?php echo $fetch['r_no']; ?></td>
